...
Awareness 作为 Pi extension 向 Agent Core 暴露两个稳定入口：

1. 先调用 `awareness_observe(action="list_adapters")`，取得当前 runtime 的 `adapter_id` 与动作清单。
2. 只读动作使用 `awareness_observe`；改变外界状态的动作使用 `awareness_interact`。
3. 两个工具都会把 Pi 的 tool call ID 写入结果 `call_id`。只接受同一 `call_id` 的结果作为该次调用回执。
