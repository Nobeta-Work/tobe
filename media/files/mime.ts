...
export function detectMimeType(data: Uint8Array, fileName?: string): { mimeType: string; kind: MediaKind } {
  if (starts(data, [0x89, 0x50, 0x4e, 0x47])) return { mimeType: "image/png", kind: "image" };
  if (starts(data, [0xff, 0xd8, 0xff])) return { mimeType: "image/jpeg", kind: "image" };
  if (ascii(data, 0, 6) === "GIF87a" || ascii(data, 0, 6) === "GIF89a") return { mimeType: "image/gif", kind: "image" };
  if (ascii(data, 0, 4) === "RIFF" && ascii(data, 8, 4) === "WEBP") return { mimeType: "image/webp", kind: "image" };
  if (ascii(data, 0, 4) === "RIFF" && ascii(data, 8, 4) === "WAVE") return { mimeType: "audio/wav", kind: "audio" };
  if (starts(data, [0x49, 0x44, 0x33]) || starts(data, [0xff, 0xfb]) || starts(data, [0xff, 0xf3])) return { mimeType: "audio/mpeg", kind: "audio" };
  if (ascii(data, 0, 4) === "OggS") return { mimeType: "audio/ogg", kind: "audio" };
  if (ascii(data, 4, 4) === "ftyp") {
    return fileName?.toLowerCase().endsWith(".m4a") ? { mimeType: "audio/mp4", kind: "audio" } : { mimeType: "video/mp4", kind: "video" };
  }
...
function starts(data: Uint8Array, signature: readonly number[]): boolean { return signature.every((value, index) => data[index] === value); }
function ascii(data: Uint8Array, start: number, length: number): string { return Buffer.from(data.subarray(start, start + length)).toString("ascii"); }
